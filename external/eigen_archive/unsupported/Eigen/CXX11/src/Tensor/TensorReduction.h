...
struct FullReducer<Self, Op, ThreadPoolDevice, Vectorizable> {
  ...
  static void run_impl(const Self& self, Op& reducer, const ThreadPoolDevice& device,
                  typename Self::CoeffReturnType* output, std::false_type) {
  ...
  static void run_impl(const Self& self, Op& reducer, const ThreadPoolDevice& device,
                  typename Self::CoeffReturnType* output, std::true_type) {
    typedef typename Self::Index Index;
    typedef typename Self::CoeffReturnType TensorReturnType;
    const Index num_coeffs = array_prod(self.m_impl.dimensions());
    if (num_coeffs == 0) {
      output = new TensorReturnType(reducer.finalize(reducer.initialize()));
      return;
    }
    const TensorOpCost cost =
        self.m_impl.costPerCoeff(Vectorizable) +
    ...
    const int num_threads = TensorCostModel<ThreadPoolDevice>::numThreads(
        num_coeffs, cost, device.numThreads());
    if (num_threads == 1) {
      output = new TensorReturnType(InnerMostDimReducer<Self, Op, Vectorizable>::reduce(self, 0, num_coeffs, reducer));
      return;
    }
    const Index blocksize =
        std::floor<Index>(static_cast<float>(num_coeffs) / num_threads);
    ...
    output = new TensorReturnType(reducer.finalize(finalShard));
  }

  static void run(const Self& self, Op& reducer, const ThreadPoolDevice& device,
                  typename Self::CoeffReturnType* output) {
    typedef typename Self::CoeffReturnType TensorReturnType;               
    run_impl(self, reducer, device, output, std::is_same<TensorReturnType, iRRAM::REAL>{});
  }
};
...
struct TensorReductionEvaluatorBase<const TensorReductionOp<Op, Dims, ArgType, MakePointer_>, Device>
{
  ...
  EIGEN_STRONG_INLINE
    ...
    EIGEN_DEVICE_FUNC
    #endif
    bool evalSubExprsIfNeeded_impl(EvaluatorPointerType data, std::true_type) {
    m_impl.evalSubExprsIfNeeded(NULL);
    std::cout<<"TensorReduction: evalSubExprsIfNeeded 1 true"<<std::endl;
    ...
    if ((RunningFullReduction && RunningOnSycl) ||(RunningFullReduction &&
      ...
      std::cout<<"TensorReduction: evalSubExprsIfNeeded 2 true"<<std::endl;
      if (!data) {
        std::cout<<"TensorReduction: evalSubExprsIfNeeded 3 true"<<std::endl;
        std::cout<<"TensorReduction: evalSubExprsIfNeeded 3 true EvaluatorPointerType real is: "<<typeid(EvaluatorPointerType).name()<<std::endl;
        iRRAM::REAL* p = nullptr;
        m_device.allocate_temp(p, 1);
        m_result = static_cast<iRRAM::REAL*>(p);
        data = m_result;
        need_assign = true;
      }

      std::cout<<"TensorReduction: evalSubExprsIfNeeded 4 true "<<std::endl;
    ...
    // Attempt to use an optimized reduction.
      ...
      std::cout<<"TensorReduction: evalSubExprsIfNeeded 5 true"<<std::endl;
      ...
      std::cout<<"TensorReduction: evalSubExprsIfNeeded 6 true"<<std::endl;
      if (internal::InnerReducer<Self, Op, Device>::HasOptimizedImplementation &&
          (reducing_inner_dims || ReducingInnerMostDims)) {
        const Index num_values_to_reduce = internal::array_prod(m_reducedDims);
        const Index num_coeffs_to_preserve = internal::array_prod(m_dimensions);
        std::cout<<"TensorReduction: evalSubExprsIfNeeded 7"<<std::endl;
        if (!data) {
          std::cout<<"TensorReduction: evalSubExprsIfNeeded 8 true"<<std::endl;
          if ((num_coeffs_to_preserve < 1024 && num_values_to_reduce > num_coeffs_to_preserve && num_values_to_reduce > 128) || (RunningOnSycl)) {
            ...
            std::cout<<"TensorReduction: evalSubExprsIfNeeded 9"<<std::endl;
            return true;
          }
        }
        Op reducer(m_reducer);
        ...
        if (internal::InnerReducer<Self, Op, Device>::run(*this, reducer, m_device, data, num_values_to_reduce, num_coeffs_to_preserve)) {
          std::cout<<"TensorReduction: evalSubExprsIfNeeded 10"<<std::endl;
          if (m_result) {
          ...
          std::cout<<"TensorReduction: evalSubExprsIfNeeded 11"<<std::endl;
          return (m_result != NULL);
        }
      }

      std::cout<<"TensorReduction: evalSubExprsIfNeeded 12"<<std::endl;
  ...
  EIGEN_STRONG_INLINE
    ...
    EIGEN_DEVICE_FUNC
    #endif
    bool evalSubExprsIfNeeded_impl(EvaluatorPointerType data, std::false_type) {
    m_impl.evalSubExprsIfNeeded(NULL);
    std::cout<<"TensorReduction: evalSubExprsIfNeeded 1"<<std::endl;
    // Use the FullReducer if possible.
    if ((RunningFullReduction && RunningOnSycl) ||(RunningFullReduction &&
        internal::FullReducer<Self, Op, Device>::HasOptimizedImplementation &&
        ((RunningOnGPU && (m_device.majorDeviceVersion() >= 3)) ||
         !RunningOnGPU))) {
      bool need_assign = false;
      std::cout<<"TensorReduction: evalSubExprsIfNeeded 2"<<std::endl;
      if (!data) {
        std::cout<<"TensorReduction: evalSubExprsIfNeeded 3"<<std::endl;
        m_result = static_cast<EvaluatorPointerType>(m_device.get((CoeffReturnType*)m_device.allocate_temp(sizeof(CoeffReturnType))));
        data = m_result;
        need_assign = true;
      }

      std::cout<<"TensorReduction: evalSubExprsIfNeeded 4"<<std::endl;
      Op reducer(m_reducer);
      internal::FullReducer<Self, Op, Device>::run(*this, reducer, m_device, data);
      return need_assign;
    }
    ...
    // Attempt to use an optimized reduction.
      ...
      std::cout<<"TensorReduction: evalSubExprsIfNeeded 5"<<std::endl;
      bool reducing_inner_dims = true;
      for (int i = 0; i < NumReducedDims; ++i) {
        if (static_cast<int>(Layout) == static_cast<int>(ColMajor)) {
          reducing_inner_dims &= m_reduced[i];
      ...
      std::cout<<"TensorReduction: evalSubExprsIfNeeded 6"<<std::endl;
      if (internal::InnerReducer<Self, Op, Device>::HasOptimizedImplementation &&
          (reducing_inner_dims || ReducingInnerMostDims)) {
        const Index num_values_to_reduce = internal::array_prod(m_reducedDims);
        const Index num_coeffs_to_preserve = internal::array_prod(m_dimensions);
        std::cout<<"TensorReduction: evalSubExprsIfNeeded 7"<<std::endl;
        if (!data) {
          std::cout<<"TensorReduction: evalSubExprsIfNeeded 8"<<std::endl;
          if ((num_coeffs_to_preserve < 1024 && num_values_to_reduce > num_coeffs_to_preserve && num_values_to_reduce > 128) || (RunningOnSycl)) {
            std::cout<<"TensorReduction: evalSubExprsIfNeeded 8"<<std::endl;
            data = static_cast<EvaluatorPointerType>(m_device.get((CoeffReturnType*)m_device.allocate_temp(sizeof(CoeffReturnType) * num_coeffs_to_preserve)));
            m_result = data;
  ...
  EIGEN_STRONG_INLINE
    ...
    bool evalSubExprsIfNeeded(EvaluatorPointerType data) {
      return evalSubExprsIfNeeded_impl(data, std::is_same<EvaluatorPointerType, iRRAM::REAL*>{});
  }
