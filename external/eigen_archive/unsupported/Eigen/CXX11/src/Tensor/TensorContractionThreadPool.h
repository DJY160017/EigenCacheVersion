...
struct TensorEvaluator<const TensorContractionOp<Indices, LeftArgType, RightArgType, OutputKernelType>, ThreadPoolDevice> :
  ...
  void evalProduct(Scalar* buffer) const {
    std::cout<<"TensorConstructionThreadPool: evalProduct enter"<<std::endl;
    ...
    if (this->m_can_use_xsmm) {
      std::cout<<"TensorConstructionThreadPool: evalProduct enter1"<<std::endl;
      ...
      if (blocking.num_threads() == 1) {
        std::cout<<"TensorConstructionThreadPool: evalProduct enter2"<<std::endl;
        this->evalGemmXSMM(buffer);
      } else {
        std::cout<<"TensorConstructionThreadPool: evalProduct enter3"<<std::endl;
    ...
    Index bm, bn, bk;
    if (shard_by_col) {
      std::cout<<"TensorConstructionThreadPool: evalProduct enter4"<<std::endl;
      ...
      std::cout<<"TensorConstructionThreadPool: evalProduct enter5"<<std::endl;
      internal::TensorContractionBlocking<Scalar, LhsScalar, RhsScalar, Index,
                                          internal::ShardByRow>
          blocking(k, m, n, 2);
      bm = blocking.mc();
    ...
    if (shardByInnerDim(m, n, k, num_threads, num_threads_by_k)) {
      std::cout<<"TensorConstructionThreadPool: evalProduct enter6"<<std::endl;
    ...
    if (num_threads == 1) {
      std::cout<<"TensorConstructionThreadPool: evalProduct enter7"<<std::endl;
    ...
    if (shard_by_col) {
      std::cout<<"TensorConstructionThreadPool: evalProduct enter8"<<std::endl;
      internal::TensorContractionBlocking<Scalar, LhsScalar, RhsScalar, Index,
                                          internal::ShardByCol>
      ...
      std::cout<<"TensorConstructionThreadPool: evalProduct enter9"<<std::endl;
      internal::TensorContractionBlocking<Scalar, LhsScalar, RhsScalar, Index,
                                          internal::ShardByRow>
          blocking(k, m, n, num_threads);
      bm = blocking.mc();
    ...
    if (shard_by_col) {
      std::cout<<"TensorConstructionThreadPool: evalProduct enter10"<<std::endl;
      gm = coarsenM(m, n, bm, bn, bk, gn, num_threads, shard_by_col);
      gn = coarsenN(m, n, bm, bn, bk, gm, num_threads, shard_by_col);
    } else {
      std::cout<<"TensorConstructionThreadPool: evalProduct enter11"<<std::endl;
      gn = coarsenN(m, n, bm, bn, bk, gm, num_threads, shard_by_col);
      gm = coarsenM(m, n, bm, bn, bk, gn, num_threads, shard_by_col);
    }
...
#undef CONTEXT_ARGS
 ...
 template <bool lhs_inner_dim_contiguous, bool rhs_inner_dim_contiguous,
          ...
          kernel_(m_, k_, n_, bm_, bk_, bn_)
  {
      ...
      std::cout<<"TensorConstructionThreadPool: allocateSlices 1 scalar: "<<std::is_same<Scalar, iRRAM::REAL>::value<<std::endl;
      ...
      if (parallelize_by_sharding_dim_only_) {
        std::cout<<"TensorConstructionThreadPool: allocateSlices 2"<<std::endl;
        const int num_worker_threads = device_.numThreadsInPool();

        if (shard_by_col) {
          std::cout<<"TensorConstructionThreadPool: allocateSlices 3"<<std::endl;
          ...
          std::cout<<"TensorConstructionThreadPool: allocateSlices 4"<<std::endl;
    ...
    ~Context() {
      ...
      // if(std::is_same<Scalar, iRRAM::REAL>::value){
      //   auto lhs_size = bm_ * bk_;
      //   auto rhs_size = bn_ * bk_;
      //   auto length = (nm0_* lhs_size+nn0_* rhs_size)*std::min<Index>(nk_, P - 1);
      //   kernel_.deallocate(device_, packed_mem_, length);
      //   if (parallelize_by_sharding_dim_only_) {
      //     const int num_worker_threads = device_.numThreadsInPool();
      //     if(shard_by_col_){
      //       kernel_.deallocate(device_, thread_local_packed_mem_, num_worker_threads * gn_*rhs_size);
      //     }else{
      //       kernel_.deallocate(device_, thread_local_packed_mem_, num_worker_threads * gm_*lhs_size);
      //     }
  ...
  void evalShardedByInnerDim(int num_threads, Scalar* result) const {
    ...
    std::cout<<"TensorConstructionThreadPool: evalShardedByInnerDim 1"<<std::endl;
