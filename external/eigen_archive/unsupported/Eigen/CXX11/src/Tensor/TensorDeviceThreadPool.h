...
struct ThreadPoolDevice {
  ...
  EIGEN_STRONG_INLINE void* allocate(size_t num_bytes) const {
    std::cout<<"TensorDeviceThreadPool: allocate"<<std::endl;
    return allocator_ ? allocator_->allocate(num_bytes)
        : internal::aligned_malloc(num_bytes);
  }

  EIGEN_STRONG_INLINE void deallocate(void* buffer) const {
    std::cout<<"TensorDeviceThreadPool: deallocate"<<std::endl;
    if (allocator_) {
  ...
  EIGEN_STRONG_INLINE void* allocate_temp(size_t num_bytes) const {
    std::cout<<"TensorDeviceThreadPool: allocate_temp"<<std::endl;
    return allocate(num_bytes);
  }

  EIGEN_STRONG_INLINE void allocate_temp(iRRAM::REAL* p, size_t n) const {
    std::cout<<"TensorDeviceThreadPool: allocate_temp iRRAM n:"<<n<<std::endl;
    for (size_t i = 0; i < n; ++p, ++i){
      new (p) iRRAM::REAL();
      auto size = sizeof(*p);
      std::cout<<"TensorDeviceThreadPool: allocate_temp iRRAM loop test: "<<i<<" "<<size<<std::endl;
    }
    std::cout<<"TensorDeviceThreadPool: allocate_temp iRRAM end"<<std::endl;
  }

  EIGEN_STRONG_INLINE void deallocate_temp(iRRAM::REAL* p, size_t n) const {
    std::cout<<"TensorDeviceThreadPool: deallocate_temp iRRAM"<<std::endl;
    for (size_t i = 0; i < n; ++p, ++i) p->~REAL();
  }

  EIGEN_STRONG_INLINE void deallocate_temp(void* buffer) const {
    std::cout<<"TensorDeviceThreadPool: deallocate_temp"<<std::endl;
    deallocate(buffer);
  }
  ...
  EIGEN_STRONG_INLINE void memcpy(void* dst, const void* src, size_t n) const {
    std::cout<<"TensorDeviceThreadPool: memcpy"<<std::endl;
#ifdef __ANDROID__
    ::memcpy(dst, src, n);
#else
  ...
  EIGEN_STRONG_INLINE void memset(void* buffer, int c, size_t n) const {
    std::cout<<"TensorDeviceThreadPool: memset"<<std::endl;
    ::memset(buffer, c, n);
  }
