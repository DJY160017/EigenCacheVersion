...
struct TensorContractionBlockMemAllocator {
  ...
  EIGEN_DEVICE_FUNC static BlockMemHandle allocateSlices(
    ...
    std::cout<<"TensorConstruction: allocateSlices 2 false"<<std::endl;
  ...
  template <typename Device>
  EIGEN_DEVICE_FUNC static void deallocate_for_real(Device& d, BlockMemHandle handle, size_t length) {
    d.deallocate_temp((iRRAM::REAL*)handle, length);
  }
 ...
 private:
  ...
  EIGEN_DEVICE_FUNC static Index read_iRRAM_size() {
    std::ifstream inFile;
    inFile.open("/home/byron/Documents/workspace/NeuralNetworFloatAbnormal/simple_network/adjust.txt");
    std::string str;
    if (inFile){
        getline(inFile,str);
        inFile.close();
  ...
  EIGEN_DEVICE_FUNC static BlockSizes ComputeLhsRhsBlockSizes_impl(const Index bm,
                                                              const Index bk,
                                                              const Index bn, std::false_type) {
  ...
  EIGEN_DEVICE_FUNC static BlockSizes ComputeLhsRhsBlockSizes_impl(const Index bm,
                                                              const Index bk,
                                                              const Index bn, std::true_type) {
    Index align = numext::maxi(EIGEN_MAX_ALIGN_BYTES, 1);
    BlockSizes sz;
    Index size  = read_iRRAM_size();
    sz.lhs_size = divup<Index>(bm * bk * sizeof(LhsScalar)*size, align) * align;
    sz.rhs_size = divup<Index>(bn * bk * sizeof(RhsScalar)*size, align) * align;
    return sz;
  }
   ...
   EIGEN_DEVICE_FUNC static BlockSizes ComputeLhsRhsBlockSizes(const Index bm,
                                                              const Index bk,
                                                              const Index bn){
      ComputeLhsRhsBlockSizes_impl(bm, bk, bn, std::is_same<LhsScalar, iRRAM::REAL>{});

  }

  EIGEN_DEVICE_FUNC static BlockSizes ComputeLhsRhsBlockSizes_for_real(const Index bm,
                                                              const Index bk,
                                                              const Index bn) {
    Index align = numext::maxi(EIGEN_MAX_ALIGN_BYTES, 1);
    BlockSizes sz;
...
struct TensorContractionKernel {
  ...
  EIGEN_DEVICE_FUNC BlockMemHandle allocateSlices(
    ...
    std::cout<<"TensorConstruction: allocateSlices 1"<<std::endl;
    return BlockMemAllocator::allocateSlices(
        d, bm, bk, bn, num_lhs, num_rhs, num_slices, lhs_blocks, rhs_blocks);
  }

  template <typename Device>
  EIGEN_DEVICE_FUNC static void deallocate(Device& d, BlockMemHandle handle, size_t length = 0) {
    // if(length == 0){
...
struct TensorContractionEvaluatorBase
{
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE bool evalSubExprsIfNeeded(EvaluatorPointerType data) {
    ...
    if (data) {
      std::cout<<"TensorConstruction: evalSubExprsIfNeeded enter 1"<<std::endl;
      evalTo(data);
      return false;
    } else {
      std::cout<<"TensorConstruction: evalSubExprsIfNeeded enter 2"<<std::endl;
...
#define TENSOR_CONTRACTION_DISPATCH(METHOD, ALIGNMENT, ARGS)   \
  ...
  void evalProductSequential(Scalar* buffer) const {
    std::cout<<"TensorConstruction: evalProductSequential enter"<<std::endl;
    if (this->m_j_size == 1) {
      std::cout<<"TensorConstruction: evalProductSequential enter 1"<<std::endl;
      this->template evalGemv<lhs_inner_dim_contiguous,
                              rhs_inner_dim_contiguous, rhs_inner_dim_reordered,
                              Alignment>(buffer);
    } else {
      std::cout<<"TensorConstruction: evalProductSequential enter 2"<<std::endl;
...
struct TensorEvaluator<const TensorContractionOp<Indices, LeftArgType, RightArgType, OutputKernelType>, Device> :
  ...
  void evalProduct(Scalar* buffer) const {
    std::cout<<"TensorConstruction: evalProduct enter"<<std::endl;
